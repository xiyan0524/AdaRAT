...
if __name__ == "__main__":
    ...
    for epoch in range(0, args.epoch):
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            train_attack = PGD.FAT_Attack(net, args.epsilon, args.step_size, args.step_num, adjust_tau(epoch))
            ...
            if args.FixRAT:
                if args.AdaRAT:
                    ...
                    opt_loss = clean_loss + FixRAT(clean_outputs, perturbed_outputs, labels)
            else:
                opt_loss = perturbed_loss
        ...
        with torch.no_grad():
            for data in test_loader:
                ...
                test_FGSM_attack = FGSM.FGSM_Attack(net, 8 / 255)
                FGSM_inputs = test_FGSM_attack.attack(inputs, labels)

                test_PGD_attack = PGD.PGD_Attack(net, 8 / 255, args.step_size, args.step_num)
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            FGSM_attack = FGSM.FGSM_Attack(net, 8 / 255)
            PGD10_attack = PGD.PGD_Attack(net, 8 / 255, args.step_size, iteration=10)
            PGD20_attack = PGD.PGD_Attack(net, 8 / 255, args.step_size, iteration=20)
            PGD50_attack = PGD.PGD_Attack(net, 8 / 255, args.step_size, iteration=50)
            # CW_attack = torchattacks.CW(net, c=0.01, kappa=0, steps=20, lr=0.1)
            CW_attack = cw.CW_Attack(net, 8 / 255, 20, args.step_size, args.class_num)
