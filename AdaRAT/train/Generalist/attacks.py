...
class AttackerPolymer:
    ...
    def run_all(self, model, img, gt, return_acc=True):
        ...
        adv_acc_dict['FGSM'] = self.FGSM(model, img, gt, return_acc=return_acc)
    ...
    def run_specified(self, name, model, img, gt, step_count=None, category='Madry', return_acc=False):
        ...
        if 'PGD' in name:
            ...
            return self.FGSM(model, img, gt, step_count=step_count, return_acc=return_acc)
        elif name == 'MIM':
    ...
    def FGSM(self, model, img, gt, step_count=None, return_acc=False):
        model.eval()
        img.requires_grad_()
        output = model(img)
        ...
        grad = torch.autograd.grad(loss_adv, [img])[0]
        eta = self.epsilon * grad.sign()
        # Update adversarial img
        x_adv = img.detach() + eta
    ...
    def CW(self, model, img, gt, margin=50, return_acc=False):
        ...
        onehot_targets = F.one_hot(gt, self.num_classes)
    ...
    def AA(self, model, img, gt, attacks_to_run=None, return_acc=False):
        ...
        adversary = AutoAttack(model, norm='Linf', eps=self.epsilon, version='standard', verbose=False, device=self.device)
        if attacks_to_run:
