...
parser.add_argument('--arch', type=str, default="WRN34",
...
parser.add_argument('--out-dir',type=str, default='../../result/cifar100/txt/',help='dir of output')
parser.add_argument('--data-dir',type=str, default='../../result/cifar100/data/',help='dir of output')
parser.add_argument('--model-dir',type=str, default='../../result/cifar100/save_model/',help='dir of output')
...
parser.add_argument('--up_lim', default=2.0, type=float, help='up limit')
            ...
            if model2 and beta:
                if i < len(self.param_keys):
                    ...
                    self.shadow[name].copy_(
                        decay * self.shadow[name] + (1 - decay) * (state[name] * beta + state2[name] * (1 - beta)))
            else:
...
def attack(model, Attackers, device):
    ...
    return [clean_accuracy.mean, fgsm_accuracy.mean, pgd10_accuracy.mean, pgd20_accuracy.mean, pgd50_accuracy.mean,
            cw_accuracy.mean, aa_accuracy.mean]
...
def main():
    ...
    if not args.resume:
        ...
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            train(epoch, model_at, model_st, teacher_at, teacher_st, teacher_mixed, Attackers, optimizer_ST,
                  optimizer_AT, device, descrip_str)
            # elapsed = round(time.time() - start_time)
            # elapsed = str(datetime.timedelta(seconds=elapsed))
            # print(elapsed)

            nat_acc, pgd20_acc, ema_nat_acc, ema_pgd20_acc = test(teacher_st.model, teacher_mixed.model, Attackers,
                                                                  device=device)
    ...
    logger_test.append(
        [1000001, res_list2[0], res_list2[1], res_list2[2], res_list2[3], res_list2[4], res_list2[5], res_list2[6]])
