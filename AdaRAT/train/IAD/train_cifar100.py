...
parser = argparse.ArgumentParser(description='IAD')
...
parser.add_argument('--step_num', default=100, type=int, help='step numbers')
parser.add_argument('--teacher_robust_name', default='PGD_AT.pth', type=str, help='dataset path')
parser.add_argument('--teacher_standard_name', default='ST.pth', type=str, help='dataset path')
parser.add_argument('--ce', default=0.25, type=float, help='ce')
parser.add_argument('--teacher_robust', default=0.5, type=float, help='teacher_robust')
parser.add_argument('--teacher_standard', default=0.25, type=float, help='teacher_standard')
parser.add_argument('--begin',type=int, default=60)
parser.add_argument('--b', default=0.1, type=float, help='beta')

parser.add_argument('--FixRAT', default=False, type=bool, help='use new regularization')
parser.add_argument('--beta', default=0.05, type=float, help='beta')

parser.add_argument('--AdaRAT', default=False, type=bool, help='use two-stage optimization')
parser.add_argument('--up_lim', default=1.8, type=float, help='up limit')
parser.add_argument('--down_lim', default=0.4, type=float, help='down limit')
parser.add_argument('--tao', default=0.01, type=float, help='tao')

args = parser.parse_args()

train_loader, test_loader = load_cifar100.load(args.dataset_path, args.batch_size, args.drop_last)

net = wide_resnet.WideResnet34_10(args.class_num).to(device)
teacher_robust = wide_resnet.WideResnet34_10(args.class_num).to(device)
teacher_standard = wide_resnet.WideResnet34_10(args.class_num).to(device)
teacher_robust.load_state_dict(torch.load("../../result/cifar100/save_model/" + args.teacher_robust_name))
teacher_standard.load_state_dict(torch.load("../../result/cifar100/save_model/" + args.teacher_standard_name))
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        ...
        level=logging.DEBUG,
        handlers=[
            logging.FileHandler(os.path.join("../../result/cifar100/txt/", method + ".log")),
            logging.StreamHandler()
        ])

    txt.clear("../../result/cifar100/txt/" + method + ".log")
    ...
    for epoch in range(0, args.epoch):
        optimizer = optim.SGD(net.parameters(), lr=adjust_Learning_Rate(epoch), momentum=0.9, weight_decay=7e-4)
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            perturbed_inputs = train_attack.attack(inputs, labels)

            net_clean_outputs = net(inputs)
            with torch.no_grad():
                teacher_robust_clean_outputs = teacher_robust(inputs)
                teacher_standard_clean_outputs = teacher_standard(inputs)

            net_perturbed_outputs = net(perturbed_inputs)
            with torch.no_grad():
                teacher_robust_perturbed_outputs = teacher_robust(perturbed_inputs)
                teacher_standard_perturbed_outputs = teacher_standard(perturbed_inputs)

            alpha = torch.ones(len(inputs)).cuda()
            guide = teacher_robust_perturbed_outputs

            ce_clean_loss = criterion(net_clean_outputs, labels)
            ce_perturbed_loss = criterion(net_perturbed_outputs, labels)
            teacher_robust_clean_loss = criterion_kl(F.log_softmax(net_clean_outputs, dim=1),
                                                     F.softmax(teacher_robust_clean_outputs, dim=1))
            teacher_robust_perturbed_loss = criterion_kl(F.log_softmax(net_perturbed_outputs, dim=1),
                                                     F.softmax(teacher_robust_perturbed_outputs, dim=1))
            teacher_standard_clean_loss = criterion_kl(F.log_softmax(net_clean_outputs, dim=1),
                                                     F.softmax(teacher_standard_clean_outputs, dim=1))
            teacher_standard_perturbed_loss = criterion_kl(F.log_softmax(net_perturbed_outputs, dim=1),
                                                         F.softmax(teacher_standard_perturbed_outputs, dim=1))

            kd_clean_loss = (args.ce * ce_clean_loss + args.teacher_robust * teacher_robust_clean_loss +
                             args.teacher_standard * teacher_standard_clean_loss)
            if epoch >= args.begin:
                for pp in range(len(net_perturbed_outputs)):
                    L = F.softmax(guide, dim=1)[pp][labels[pp].item()]
                    L = L.pow(args.b).item()
                    alpha[pp] = L
                kd_perturbed_loss = (args.ce * criterion(net_perturbed_outputs, labels) +
                                    args.teacher_robust * ((1 / len(net_perturbed_outputs)) * torch.sum(KL(F.log_softmax(net_perturbed_outputs, dim=1),F.softmax(teacher_robust_perturbed_outputs, dim=1)).sum(dim=1).mul(alpha)) +
                                                            (1 / len(net_perturbed_outputs)) * torch.sum(KL(F.log_softmax(net_perturbed_outputs, dim=1), F.softmax(net_clean_outputs, dim=1)).sum(dim=1).mul(1 - alpha))) +
                                    args.teacher_standard * (1 / len(net_perturbed_outputs)) * torch.sum(KL(F.log_softmax(net_perturbed_outputs, dim=1),F.softmax(teacher_standard_perturbed_outputs, dim=1)).sum(dim=1)))
            else:
                kd_perturbed_loss = (args.ce * ce_perturbed_loss + args.teacher_robust * teacher_robust_perturbed_loss +
                             args.teacher_standard * teacher_standard_perturbed_loss)

            if args.FixRAT:
                if args.AdaRAT:
                    if kd_clean_loss >= args.up_lim * 0.7:
                        opt_loss = kd_clean_loss
                    else:
                        opt_loss = (torch.pow(
                            torch.max(
                                torch.zeros_like(kd_clean_loss),
                                torch.tan(
                                    math.pi * kd_clean_loss / (2 * (args.up_lim - args.down_lim)) -
                                    math.pi * args.down_lim / (2 * (args.up_lim - args.down_lim))
                                )
                            ),
                            1.0 + args.tao)
                                    + args.beta / args.epsilon * (kd_perturbed_loss - kd_clean_loss))
                else:
                    opt_loss = kd_clean_loss + args.beta / args.epsilon * (kd_perturbed_loss - kd_clean_loss)
            else:
                opt_loss = kd_perturbed_loss
            ...
            train_clean_loss += kd_clean_loss.item()
            _, clean_predicted = torch.max(net_clean_outputs.data, 1)
            train_clean_acc += clean_predicted.eq(labels.data).cpu().sum()

            train_ae_loss += kd_perturbed_loss.item()
            _, perturbed_predicted = torch.max(net_perturbed_outputs.data, 1)
    ...
    torch.save(net.state_dict(), "../../result/cifar100/save_model/" + method + ".pth")
    np.save("../../result/cifar100/data/" + method + "_clean.npy", Clean)
    np.save("../../result/cifar100/data/" + method + "_pgd.npy", Pgd)
