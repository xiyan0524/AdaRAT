...
parser = argparse.ArgumentParser(description='KD AT')
...
parser.add_argument('--class_num', default=100, type=int, help='class number')
...
parser.add_argument('--up_lim', default=1.4, type=float, help='up limit')
parser.add_argument('--down_lim', default=0.8, type=float, help='down limit')
...
if __name__ == "__main__":
    ...
    title = "epoch   learning_rate   train_time   train_AE_loss   train_AE_acc   train_clean_loss   train_clean_acc " \
            "  test_clean_loss   test_clean_acc   test_FGSM_loss   test_FGSM_acc   test_PGD_loss   test_PGD_acc   teacher"
    logger.info(title)
    ...
    for epoch in range(0, args.epoch):
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            _, teacher_predicted = torch.max(teacher_robust_perturbed_outputs.data, 1)
            train_teacher_ae_acc += teacher_predicted.eq(labels.data).cpu().sum()
        ...
        train_teacher_ae_acc = train_teacher_ae_acc / total * 100.0
        ...
        logger.info("%03d     %.4f          %06.2f       %07.4f         %05.2f%%         %07.4f            %05.2f%%"
                    ...
                    % (epoch + 1, adjust_Learning_Rate(epoch), end_time - start_time, train_ae_loss, train_ae_acc,
                       ...
                       test_FGSM_acc,
                       test_PGD_loss, test_PGD_acc, train_teacher_ae_acc)
                    )
